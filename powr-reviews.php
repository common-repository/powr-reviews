<?php
    /**
     * @package POWr Reviews
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Reviews
    Plugin URI: https://www.powr.io/plugins/reviews
    Description: Collect and display social proof with visitor reviews  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Reviews extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_reviews',
                             __( 'POWr Reviews' ),
                             array( 'description' => __( 'Reviews by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-reviews' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Reviews.
        </p>
        <p>
    ...
    function register_powr_reviews() {
      register_widget( 'Powr_Reviews' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_reviews' );
    //Create short codes for adding plugins anywhere:
    function powr_reviews_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-reviews' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-reviews' label='$label'></div>";
      }else{
      	"<div class='powr-reviews'></div>";
      }
    }
    add_shortcode( 'powr-reviews', 'powr_reviews_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_reviews_menu' );
    function powr_reviews_menu() {
      add_menu_page( 'POWr Reviews', 'POWr Reviews', 'manage_options', 'powr-reviews-settings', 'powr_reviews_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_reviews_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/reviews';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/reviews';
      }
    ...
    register_activation_hook(__FILE__, 'powr_reviews_plugin_activate');
      add_action('admin_init', 'powr_reviews_plugin_redirect');

      function powr_reviews_plugin_activate() {
      add_option('powr_reviews_plugin_do_activation_redirect', true);
      }
      ...
      function powr_reviews_plugin_redirect() {
      if (get_option('powr_reviews_plugin_do_activation_redirect', false)) {
          delete_option('powr_reviews_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-reviews-settings&' );
          }
       }
      }
